# IBM Digital Business Automation - Decision Manager Open Edition - Adaptors

This repository contains various product extensions, adaptors, and application programming interfaces (API's) to be used in conjunction with `IBM Decision Manager Open Edition` projects.
...
1.  [**Embedded Engine Adaptor**](./embedded-engine-adaptor/readme.md) - A Java adaptor written around the embedded rule engine API, used in situations where the application contains the rule engine and the ruleset all packaged together, rather than delegate rule execution to an external engine, such as the KIE Server.
2.  [**KIE Server Adaptor**](./kie-server-adaptor.readme.md) - A sample adaptor to the KIE Server's REST API, used to augment the REST API and other internval behavior of the KIE Server.
